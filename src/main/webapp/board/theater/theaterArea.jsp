...
<%
String tsortParam = request.getParameter("tsort");
String tPageParam = request.getParameter("tPage");

if (tsortParam != null && !tsortParam.isEmpty()) {
    session.setAttribute("tsort", tsortParam);
}
if (tPageParam != null && !tPageParam.isEmpty()) {
	session.setAttribute("tPage", tPageParam);
}
    // API 호출 및 XML 파싱
    String serviceKey = "cKJJepR6t5fqno1sj6HBXC7eGXMU4lc5Fgh5895eaEBfkz7one0Zt9hHVxkdg983tk5IYVXs8wYF5%2FabbHvzow%3D%3D";
    String realmCode = "A000";
    String sortkey = "1";
    String iPage = "1";
   	...
   	if (session.getAttribute("tsort") != null) {
   	    sortkey = (String) session.getAttribute("tsort");
   	}
   	if (session.getAttribute("tPage") != null) {
   		iPage = (String) session.getAttribute("tPage");
   	}
...
<div class="area-d">
<p class="ma">지역 : </p>
<a href="theater.jsp" class="ma">전체</a>
<a href="theaterArea.jsp?sido=서울&tPage=1&tsort=1&realmCode=<%= realmCode %>" class="ma">서울</a>
<a href="theaterArea.jsp?sido=경기&tPage=1&tsort=1&realmCode=<%= realmCode %>" class="ma">경기</a>
<a href="theaterArea.jsp?sido=충북&tPage=1&tsort=1&realmCode=<%= realmCode %>" class="ma">충북</a>
<a href="theaterArea.jsp?sido=충남&tPage=1&tsort=1&realmCode=<%= realmCode %>" class="ma">충남</a>
<a href="theaterArea.jsp?sido=전북&tPage=1&tsort=1&realmCode=<%= realmCode %>" class="ma">전북</a>
<a href="theaterArea.jsp?sido=전남&tPage=1&tsort=1&realmCode=<%= realmCode %>" class="ma">전남</a>
<a href="theaterArea.jsp?sido=경북&tPage=1&tsort=1&realmCode=<%= realmCode %>" class="ma">경북</a>
<a href="theaterArea.jsp?sido=경남&tPage=1&tsort=1&realmCode=<%= realmCode %>" class="ma">경남</a>
<a href="theaterArea.jsp?sido=인천&tPage=1&tsort=1&realmCode=<%= realmCode %>" class="ma">인천</a>
<a href="theaterArea.jsp?sido=부산&tPage=1&tsort=1&realmCode=<%= realmCode %>" class="ma">부산</a>
<a href="theaterArea.jsp?sido=울산&tPage=1&tsort=1&realmCode=<%= realmCode %>" class="ma">울산</a>
<a href="theaterArea.jsp?sido=대구&tPage=1&tsort=1&realmCode=<%= realmCode %>" class="ma">대구</a>
<a href="theaterArea.jsp?sido=강원&tPage=1&tsort=1&realmCode=<%= realmCode %>" class="ma">강원</a>
<a href="theaterArea.jsp?sido=제주&tPage=1&tsort=1&realmCode=<%= realmCode %>" class="ma">제주</a>
<div class = "sort-d">
<a href="theater.jsp?tsort=1&realmCode=<%= realmCode %>" class ="la">등록일순</a>
<a href="theater.jsp?tsort=2&realmCode=<%= realmCode %>" class ="la">공연명순</a>
  ...
  <div class="pagination">
        ...
        <!--  이전 페이지 링크 -->
            <% if (startPage > blockPage) { %>
                <li><a href="theaterArea.jsp?sido=<%= selectsido %>&mPage=<%= startPage - blockPage %>&msort=<%=sortkey%>&realmCode=<%=realmCode%>">&laquo;</a></li>
            <% } else { %>
            	<li><a href="theaterArea.jsp?sido=<%= selectsido %>&mPage=1&msort=<%=sortkey%>&realmCode=<%=realmCode%>">&laquo;</a></li>
			<% } %>
            ...
            <% for (int i = startPage; i <= endPage; i++) { %>
                ...
                <% } else { %>
                    <li><a href="theaterArea.jsp?sido=<%= selectsido %>&mPage=<%= i %>&msort=<%=sortkey%>&realmCode=<%=realmCode%>"><%= i %></a></li>
                <% } %>
            <% } %>
				
				<!--  다음 페이지 링크 -->
            <% if (endPage < totalPage) { %>
                <li><a href="theaterArea.jsp?sido=<%= selectsido %>&mPage=<%= startPage + blockPage %>&msort=<%=sortkey%>&realmCode=<%=realmCode%>">&raquo;</a></li>
            <% } else { %>
            <li><a href="theaterArea.jsp?sido=<%= selectsido %>&mPage=<%= totalPage %>&msort=<%=sortkey%>&realmCode=<%=realmCode%>">&raquo;</a></li>
            <% } %>
        </ul>
    </div>
<br>
