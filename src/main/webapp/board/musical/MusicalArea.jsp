  ...
  <div class="pagination">
        ...
        <!--  이전 페이지 링크 -->
            <% if (startPage > blockPage) { %>
                <li><a href="MusicalArea.jsp?sido=<%= selectsido %>&tPage=<%= startPage - blockPage %>&tsort=<%=sortkey%>&realmCode=<%=realmCode%>">&laquo;</a></li>
            <% } else { %>
            	<li><a href="MusicalArea.jsp?sido=<%= selectsido %>&tPage=1&tsort=<%=sortkey%>&realmCode=<%=realmCode%>">&laquo;</a></li>
			<% } %>
            ...
            <% for (int i = startPage; i <= endPage; i++) { %>
                ...
                <% } else { %>
                    <li><a href="MusicalArea.jsp?sido=<%= selectsido %>&tPage=<%= i %>&tsort=<%=sortkey%>&realmCode=<%=realmCode%>"><%= i %></a></li>
                <% } %>
            <% } %>
				
				<!--  다음 페이지 링크 -->
            <% if (endPage < totalPage) { %>
                <li><a href="MusicalArea.jsp?sido=<%= selectsido %>&tPage=<%= startPage + blockPage %>&tsort=<%=sortkey%>&realmCode=<%=realmCode%>">&raquo;</a></li>
            <% } else { %>
            <li><a href="MusicalArea.jsp?sido=<%= selectsido %>&tPage=<%= totalPage %>&tsort=<%=sortkey%>&realmCode=<%=realmCode%>">&raquo;</a></li>
            <% } %>
        </ul>
    </div>
<br>
