...
<%
String sortParam = request.getParameter("esort");
String ePageParam = request.getParameter("ePage");
if (sortParam != null && !sortParam.isEmpty()) {
    session.setAttribute("esort", sortParam);
}
if (ePageParam != null && !ePageParam.isEmpty()) {
	session.setAttribute("ePage", ePageParam);
}
...
if (session.getAttribute("esort") != null) {
    sortkey = (String) session.getAttribute("esort");
}
if (session.getAttribute("ePage") != null) {
	iPage = (String) session.getAttribute("ePage");
}
...
<link href="Extibition.css" rel="stylesheet" type="text/css" />
...
<div class="area-d">
<p class="ma">지역 : </p>
<a href="exhibition.jsp" class="ma">전체</a>
<a href="exhibitionArea.jsp?sido=서울&ePage=1&esort=1&realmCode=<%=realmCode %>" class="ma">서울</a>
<a href="exhibitionArea.jsp?sido=경기&ePage=1&esort=1&realmCode=<%=realmCode %>" class="ma">경기</a>
<a href="exhibitionArea.jsp?sido=충북&ePage=1&esort=1&realmCode=<%=realmCode %>" class="ma">충북</a>
<a href="exhibitionArea.jsp?sido=충남&ePage=1&esort=1&realmCode=<%=realmCode %>" class="ma">충남</a>
<a href="exhibitionArea.jsp?sido=전북&ePage=1&esort=1&realmCode=<%=realmCode %>" class="ma">전북</a>
<a href="exhibitionArea.jsp?sido=전남&ePage=1&esort=1&realmCode=<%=realmCode %>" class="ma">전남</a>
<a href="exhibitionArea.jsp?sido=경북&ePage=1&esort=1&realmCode=<%=realmCode %>" class="ma">경북</a>
<a href="exhibitionArea.jsp?sido=경남&ePage=1&esort=1&realmCode=<%=realmCode %>" class="ma">경남</a>
<a href="exhibitionArea.jsp?sido=인천&ePage=1&esort=1&realmCode=<%=realmCode %>" class="ma">인천</a>
<a href="exhibitionArea.jsp?sido=부산&ePage=1&esort=1&realmCode=<%=realmCode %>" class="ma">부산</a>
<a href="exhibitionArea.jsp?sido=울산&ePage=1&esort=1&realmCode=<%=realmCode %>" class="ma">울산</a>
<a href="exhibitionArea.jsp?sido=대구&ePage=1&esort=1&realmCode=<%=realmCode %>" class="ma">대구</a>
<a href="exhibitionArea.jsp?sido=강원&ePage=1&esort=1&realmCode=<%=realmCode %>" class="ma">강원</a>
<a href="exhibitionArea.jsp?sido=제주&ePage=1&esort=1&realmCode=<%=realmCode %>" class="ma">제주</a>
<div class = "sort-d">
<a href="exhibitionArea.jsp?esort=1&realmCode=<%= realmCode %>" class ="la">등록일순</a>
<a href="exhibitionArea.jsp?esort=2&realmCode=<%= realmCode %>" class ="la">공연명순</a>
...
<div class="card">
    ...
    <form action = "exhibitionDetail.jsp" method = "post">
  ...
  <div class="pagination">
        ...
        <!--  이전 페이지 링크 -->
            <% if (startPage > blockPage) { %>
                <li><a href="exhibitionArea.jsp?sido=<%= selectsido %>&ePage=<%= startPage - blockPage %>&esort=<%=sortkey%>&realmCode=<%=realmCode%>">&laquo;</a></li>
            <% } else { %>
            	<li><a href="exhibitionArea.jsp?sido=<%= selectsido %>&ePage=1&esort=<%=sortkey%>&realmCode=<%=realmCode%>">&laquo;</a></li>
			<% } %>
            ...
            <% for (int i = startPage; i <= endPage; i++) { %>
                ...
                <% } else { %>
                    <li><a href="exhibitionArea.jsp?sido=<%= selectsido %>&ePage=<%= i %>&esort=<%=sortkey%>&realmCode=<%=realmCode%>"><%= i %></a></li>
                <% } %>
            <% } %>
				
				<!--  다음 페이지 링크 -->
            <% if (endPage < totalPage) { %>
                <li><a href="exhibitionArea.jsp?sido=<%= selectsido %>&ePage=<%= startPage + blockPage %>&esort=<%=sortkey%>&realmCode=<%=realmCode%>">&raquo;</a></li>
            <% } else { %>
            <li><a href="exhibitionArea.jsp?sido=<%= selectsido %>&ePage=<%= totalPage %>&esort=<%=sortkey%>&realmCode=<%=realmCode%>">&raquo;</a></li>
            <% } %>
        </ul>
    </div>
<br>
