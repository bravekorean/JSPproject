...
<%
String esortParam = request.getParameter("esort");
String ePageParam = request.getParameter("ePage");

if (esortParam != null && !esortParam.isEmpty()) {
    session.setAttribute("esort", esortParam);
}
...
<div class = "sort-d">
<a href="exhibition.jsp?esort=1&realmCode=<%= realmCode %>" class ="la">등록일순</a>
<a href="exhibition.jsp?esort=2&realmCode=<%= realmCode %>" class ="la">공연명순</a>
   ...
   <div class="pagination">
       ...
       <!-- 이전 페이지 링크 -->
        <% if (startPage > blockPage) { %>
            <li><a href="exhibition.jsp?ePage=<%= startPage - blockPage %>&esort=<%=sortkey%>&realmCode=<%=realmCode%>">&laquo;</a></li>
        <% } else { %>
            <li><a href="exhibition.jsp?ePage=1&esort=<%=sortkey%>&realmCode=<%=realmCode%>">&laquo;</a></li>
        <% } %>
        ...
        <% for (int i = startPage; i <= endPage; i++) { %>
            ...
            <% } else { %>
                <li><a href="exhibition.jsp?ePage=<%= i %>&esort=<%=sortkey%>&realmCode=<%=realmCode%>"><%= i %></a></li>
            <% } %>
        <% } %>

        <!-- 다음 페이지 링크 -->
        <% if (endPage < totalPage) { %>
            <li><a href="exhibition.jsp?ePage=<%= startPage + blockPage %>&esort=<%=sortkey%>&realmCode=<%=realmCode%>">&raquo;</a></li>
        <% } else { %>
            <li><a href="exhibition.jsp?ePage=<%= totalPage %>&esort=<%=sortkey%>&realmCode=<%=realmCode%>">&raquo;</a></li>
        <% } %>
        </ul>
    </div>
<br>
